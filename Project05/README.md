 We know that, by default Ansible executes each task on the managed nodes only if that particular task is complete and successful.

for example, there are 3 managed nodes or servers, and we have 2 different tasks to execute on these 3 managed nodes.

According to Ansible execution nature, 
1. If first task is complete and successful on all three nodes, then only it will execute second task.
...
To handle these type of errors in ansible, we use [ignore_errors: yes]



TASK: Install Docker by following these steps. (use error handling concept to ignore errors and execute tasks)

Task 1: Check if openssh & openssl packages exists on the managed nodes. If yes, then make sure packages are latest version. If not, then no worries. just ignore.
